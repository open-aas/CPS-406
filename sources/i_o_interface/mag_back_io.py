"""IOInterface — Estação 4: MAGBACK (Magazine Traseiro)."""
...
# Mesmo módulo CP-AM-MAG do MAGFRONT — I/O idêntico
...
class _MagBackService(MicroService):
    def __init__(self, ext: "MagBackIOInterface") -> None:
    ...
    @MicroService.action
    async def mag_back_push_order(self, service, data: dict) -> None:
        await self.queue_order.put(Order(**data["order"]))

    @MicroService.action
    async def magback_downstream_ready(self, service, data: dict) -> None:
        """Chamado pela MPress quando está pronta para receber o próximo carrier."""
    ...
    async def task_process_orders(self) -> None:
        ...
        while True:
            ...
            logger.info("magback.order.start", order_id=order.order_id)
            await self._ext._cycle(order, self)
            logger.info("magback.order.done", order_id=order.order_id)


class MagBackIOInterface(PLCIOBase):
    _PLC_URL_DEFAULT = "opc.tcp://172.21.6.1:4840"
    ...
    def __init__(self, context: SubmodelContext) -> None:
        ...
        self._service   = _MagBackService(self)
    ...
    async def _on_connected(self) -> None:
        ...
        self._service.request_action("idrill_downstream_ready", {})
    ...
    async def _cycle(self, order: Order, service: _MagBackService) -> None:
        det = self._detector
        ...
        # Aguarda MPress sinalizar que está pronta antes de liberar o carrier
        ...
        logger.info("magback.order.finished", order_id=order.order_id)
        service.request_action("manager_update_has_product", {"has_product": False})
        service.request_action("mag_press_push_order", {"order": order.model_dump(mode="json")})
        # Sinaliza IDrill que MagBack está pronta para o próximo carrier
        service.request_action("idrill_downstream_ready", {})
