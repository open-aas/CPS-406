"""
IOInterface extension — iDRILL (Estação 3: Furadeira CNC)

Controla o conveyor da estação de furação. O spindle (eixos X/Z) é
acionado via MQTT e esta extensão aguarda a confirmação antes de
liberar o portador para a próxima estação.

Env vars:
  PLC_URL    opc.tcp://172.21.3.1:4840  (override)
...
_PLC_URL    = "opc.tcp://172.21.3.1:4840"
_PLC_DEVICE = ["2:DeviceSet", "3:plciDrill"]
_MQTT_SELF   = "cp406/orders/IDRILL"
_MQTT_NEXT   = "cp406/orders/MAGBACK"
_MQTT_DRILL  = "cp406/drill/trigger"    # dispara o spindle
_MQTT_DONE   = "cp406/drill/done"       # spindle sinaliza conclusão

_INPUTS: Dict[str, Optional[str]] = {
    "3:xBG1_BCD0": "Conveyor/Sensors/BG1_CarrierPresence",
}
    ...
    async def datachange_notification(self, node: Node, val, data) -> None:
        ...
        if meta:
            try:
                ...
                logger.warning("idrill.mirror.error", node=nid, error=str(exc))
...
class IOInterface(ISubmodelExtension):
    """IOInterface — Estação 3 iDRILL (conveyor + acionamento do spindle)."""
    ...
    def __init__(self, context: SubmodelContext) -> None:
        ...
        self._drill_done: asyncio.Event  = asyncio.Event()
    ...
    async def init(self) -> None:
        ...
        logger.info("idrill.plc.connected", url=self._plc_url)
        ...
        det = EdgeDetector(stopper.nodeid, EdgeType.RISING, enable=False)
        ...
        # ── MQTT ─────────────────────────────────────────────────────────
        self._mqtt = MQTTClient("faaster-idrill")
        self._mqtt.on_message = self._on_message
        await self._mqtt.connect(self._mqtt_host, self._mqtt_port)
        self._mqtt.subscribe(_MQTT_SELF, qos=1)
        self._mqtt.subscribe(_MQTT_DONE, qos=1)
        logger.info("idrill.mqtt.subscribed", topics=[_MQTT_SELF, _MQTT_DONE])

        self._tasks.append(asyncio.create_task(self._process_orders()))
        logger.info("idrill.io_interface.ready")
    ...
    def _on_message(self, client, topic, payload, qos, properties) -> None:
        if topic == _MQTT_SELF:
            try:
                ...
                logger.error("idrill.order.parse_error", error=str(exc))
        elif topic == _MQTT_DONE:
            self._drill_done.set()
            logger.info("idrill.spindle.done")
    ...
    async def _process_orders(self) -> None:
        ...
        while True:
            ...
            logger.info("idrill.order.start", order_id=order.get("order_id"))
            await self._cycle(order)
            logger.info("idrill.order.done",  order_id=order.get("order_id"))
    ...
    async def _cycle(self, order: dict) -> None:
        ...
        # dispara o spindle (mag_upper_drill) e aguarda conclusão
        self._drill_done.clear()
        self._mqtt.publish(_MQTT_DRILL, json.dumps(order).encode(), qos=1)
        logger.info("idrill.spindle.triggered")
        await self._drill_done.wait()
        ...
        # encaminha pedido para MAGBACK
        self._mqtt.publish(_MQTT_NEXT, json.dumps(order).encode(), qos=1)
        logger.info("idrill.order.forwarded", to="MAGBACK",
